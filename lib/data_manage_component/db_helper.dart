...
class SQLHelper {
  ...
  */
  ...
  static Future<List<Map<String, dynamic>>> getMonthData(DateTime createdAt) async {
    final db = await SQLHelper.db();
    return db.query('data', where: 'createdAt = ?', whereArgs: [createdAt], limit: 1);
  }
  // 전체 데이터 중 특정 일에 대한 데이터만 추출해주는 함수 (where 조건 createdAt 이용해서 캘린더에서 선택한 일에 대한 데이터 추출)
  static Future<List<Map<String, dynamic>>> getDayData(DateTime createdAt) async {
    final db = await SQLHelper.db();
    return db.query('data', where: 'createdAt = ?', whereArgs: [createdAt], limit: 1);
  }
  ...
  static Future<int> updateDescData(DateTime createdAt, String? thanksDesc,
    ...
    final result =
    await db.update('data', data, where: 'createdAt = ?', whereArgs: [createdAt]);
    return result;
  }
}
